...
func _ready():
	...
	if is_next_piece == false:
		...
		wall_kicks = Shared.wall_kicks_i if tetromino_data.tetromino_type  \
		== Shared.Tetromino.I else Shared.wall_kicks_jlostz
		ghost_tetromino = ghost_tetromino_scene.instantiate() as ghost_tetromino
...
func is_colliding_with_other_tetrominos(direction: Vector2, starting_global_position: Vector2): 
	for tetromino_piece in other_tetrominos_pieces:
			for piece in pieces:
				...
				==  + tetromino_piece.global_position:
					return true
	return false
...
func apply_rotation(direction:int):
	var rotation_matrix = Shared.clockwise_rotation_matrix if direction == 1 \
